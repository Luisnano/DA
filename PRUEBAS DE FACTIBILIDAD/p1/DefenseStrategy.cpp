...
bool factibilidad(Defense* defensa, List<Object*> obstacles, List<Defense*> defenses, 
                  bool **freeCells,float mapHeight, float mapWidth,int nCellsWidth,
                  int nCellsHeight, float map_inner_bound, int attempts) {
    ...
    bool esValido = true;
    //List<Object*> objeto_cercano = getClosestTo();

    /*if (attempts == 1000){
        List<Object*>::iterator initialObstacles = obstacles.begin();
        while (initialObstacles != obstacles.end()){
        ...
        //Ya que es la primera defensa no queremos que revise mas cosas asi que return y asi hacemos que se ponga
        return esValido;
    }*/

    //Primero: ¿Está dentro de los limites del mapa?
    /*
    if ((defensa->position.x).subtract(map.max_size) > 0  ||defensa->position.subtract(mapHeight) > 0){
        std::cout<<"DEFENSA FUERA DE MAPA"<<std::endl;
        esValido = false;
    }//if*/

    if (defensa->position.x < 30 || defensa->position.y < 30){
        std::cout<<"FUERA DE INNER BOUND"<<std::endl;
        return false;
    }

    //Ya de paso vamos a mirar que no estamos intentando colocarlo en una posicion negativa
    if (defensa->position.x + defensa->radio < 0 ||defensa->position.y + defensa->radio < 0){
        std::cout<<"DEFENSA EN POSICION NEGATIVA"<<std::endl;
        esValido = false;
    } 
    ...
    while (currentDefense != defenses.end() && esValido != false)
    {
        if((*currentDefense)->position.x == defensa->position.x && (*currentDefense)->position.y == defensa->position.y ){
            std::cout<<"DEFENSA EN POSICION DE OTRA"<<std::endl;
            esValido = false;
        }//if
        ++currentDefense;
    }//while
    ...
    while (currentObstacle != obstacles.end() && esValido != false)
    {
        if((*currentObstacle)->position.x == defensa->position.x && (*currentObstacle)->position.y == defensa->position.y){
            std::cout<<"CHOCA CON OBSTACULO"<<std::endl;
            esValido =  false;
        }//if
...
void DEF_LIB_EXPORTED placeDefenses(bool** freeCells, int nCellsWidth, int nCellsHeight, float mapWidth, float mapHeight
    ...
    while(currentDefense != defenses.end() && maxAttemps > 0) {
        ...
        bool es_factible = factibilidad((*currentDefense), obstacles, defenses, freeCells, mapHeight, mapWidth, nCellsWidth, nCellsHeight, 30.0, maxAttemps);
